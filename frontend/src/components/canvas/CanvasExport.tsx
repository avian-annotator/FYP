...
import { ConnectingLineProps } from './Objects/ConnectingLine'
...
function getKeypointAnnotation(state: CanvasState): {
  ...
  const keypointElements = state.canvasElements.filter(el => el.type === 'Keypoint')
  const lineElements = state.canvasElements.filter(el => el.type === 'ConnectingLine')
  ...
  const lines: [number, number][] = lineElements.map(line => {
    const props = line.props as ConnectingLineProps & { startId: number; endId: number }
    return [props.startId, props.endId]
  })
  ...
  const bboxAnnotations = state.canvasElements
    ...
    .map(el => {
      return {
        ...
        category_id: 1,
        bbox: [
          el.props.ref.current!.x(),
          el.props.ref.current!.y(),
          el.props.ref.current!.width(),
          el.props.ref.current!.height(),
