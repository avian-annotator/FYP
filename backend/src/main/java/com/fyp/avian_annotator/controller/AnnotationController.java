...
public class AnnotationController {
    ...
    public AnnotationActionPayload annotate(
        ...
        if (Objects.equals(message.actionType(), "join")) {
            List<Long> users =
                    activeSessions.getOrDefault(imageId, new ArrayList<>());
            ...
            activeSessions.put(imageId, users);

        } else if (Objects.equals(message.actionType(), "leave")) {
            var users = activeSessions.getOrDefault(imageId, new ArrayList<>());

            // Check needed as react runs cleanup, so it will always send and leave at the initial mount
            if (users.contains(userId)) {
                users.remove(userId);
                if (users.isEmpty()) {
                    webClient()
