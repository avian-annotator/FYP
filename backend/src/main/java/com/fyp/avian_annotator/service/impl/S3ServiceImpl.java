...
public class S3ServiceImpl implements S3Service {
    ...
    try {
      s3Client.putObject(
          software.amazon.awssdk.services.s3.model.PutObjectRequest.builder()
              ...
              .metadata(metadata)
              .build(),
          software.amazon.awssdk.core.sync.RequestBody.fromInputStream(fileContent, size));
    } catch (S3Exception e) {
      log.error("Failed to upload file {} to bucket {}: {}", key, bucketName, e.getMessage());
      throw e;
    } catch (Exception e) {
      log.error(
          "Unexpected error while uploading file {} to bucket {}: {}",
          key,
          bucketName,
          e.getMessage());
  ...
  public String createGetPresignedUrl(String key) {
    ...
    GetObjectPresignRequest getPresignRequest =
